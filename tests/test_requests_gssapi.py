...
from requests_gssapi import REQUIRED, SPNEGO
...
class GSSAPITestCase(unittest.TestCase):
    ...
    def test_negotate_value_extraction(self):
        ...
        self.assertEqual(requests_gssapi.gssapi_._negotiate_value(response), b"token")

    def test_negotate_value_extraction_none(self):
        response = requests.Response()
        response.headers = {}
        self.assertTrue(requests_gssapi.gssapi_._negotiate_value(response) is None)
    ...
    def test_force_preemptive(self):
        with patch.multiple("gssapi.SecurityContext", __init__=fake_init, step=fake_resp):
            ...
            self.assertEqual(request.headers.get("Authorization"), b64_negotiate_response)
    ...
    def test_generate_request_header_init_error(self):
        with patch.multiple("gssapi.SecurityContext", __init__=fake_init, step=fail_resp):
            response = requests.Response()
            response.url = "http://www.example.org/"
            response.headers = {"www-authenticate": b64_negotiate_token}
            host = urlparse(response.url).hostname
    ...
    def test_authenticate_user(self):
        with patch.multiple("gssapi.SecurityContext", __init__=fake_init, step=fake_resp):
            ...
            self.assertEqual(request.headers["Authorization"], b64_negotiate_response)
    ...
    def test_authenticate_server(self):
        with patch.multiple("gssapi.SecurityContext", __init__=fake_init, step=fake_resp):
            ...
            response_ok.url = "http://www.example.org/"
            response_ok.status_code = 200
            response_ok.headers = {"www-authenticate": b64_negotiate_server, "authorization": b64_negotiate_response}
    ...
    def test_handle_other(self):
        with patch.multiple("gssapi.SecurityContext", __init__=fake_init, step=fake_resp):
            ...
            auth = requests_gssapi.HTTPKerberosAuth(mutual_authentication=REQUIRED)
            auth.context = {"www.example.org": gssapi.SecurityContext}
    ...
    def test_handle_response_200_mutual_auth_required_failure(self):
        with patch.multiple("gssapi.SecurityContext", __init__=fake_init, step=fake_resp):
            ...
            self.assertRaises(requests_gssapi.MutualAuthenticationError, auth.handle_response, response_ok)
    ...
    def test_delegation(self):
        with patch.multiple("gssapi.SecurityContext", __init__=fake_init, step=fake_resp):
            ...
            response._content = b""
            response.raw = raw
            auth = requests_gssapi.HTTPKerberosAuth(service="HTTP", delegate=True)
            r = auth.authenticate_user(response)
    ...
    def test_principal_override(self):
        with patch.multiple("gssapi.Credentials", __new__=fake_creds), patch.multiple(
            "gssapi.SecurityContext", __init__=fake_init, step=fake_resp
        ):
            response = requests.Response()
            response.url = "http://www.example.org/"
            response.headers = {"www-authenticate": b64_negotiate_token}
    ...
    def test_realm_override(self):
        with patch.multiple("gssapi.SecurityContext", __init__=fake_init, step=fake_resp):
            ...
            response.headers = {"www-authenticate": b64_negotiate_token}
            host = urlparse(response.url).hostname
            auth = requests_gssapi.HTTPKerberosAuth(hostname_override="otherhost.otherdomain.org")
            auth.generate_request_header(response, host)
            fake_init.assert_called_with(
    ...
    def test_target_name(self):
        with patch.multiple("gssapi.SecurityContext", __init__=fake_init, step=fake_resp):
            ...
            response.headers = {"www-authenticate": b64_negotiate_token}
            host = urlparse(response.url).hostname
            auth = requests_gssapi.HTTPSPNEGOAuth(target_name="HTTP@otherhost.otherdomain.org")
            auth.generate_request_header(response, host)
            fake_init.assert_called_with(
                name=gssapi_sname("HTTP@otherhost.otherdomain.org"),
                usage="initiate",
